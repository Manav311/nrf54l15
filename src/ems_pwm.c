...
int ems_pwm_update_params(uint8_t chan_number, uint64_t on_time_ns, uint64_t duration_ns, uint64_t interval_ns) {
    ...
    switch (chan_number) {
        ...
        case 2: {
            ...
            temp->pwm_on_time_ns = (uint64_t)(((float)on_time_ns * PWM_USEC(ems_intensity_data.PWM23_DURATION)) / 100.0);
